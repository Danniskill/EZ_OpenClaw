...
```text
EZ_install_openclaw/
├── package.json               # Cấu hình dự án, scripts chạy/đóng gói và khai báo dependencies.
├── tailwind.config.js         # Cấu hình Tailwind CSS, liên kết các lớp màu sắc win11 vào biến CSS động.
├── postcss.config.js          # Cấu hình bộ tiền xử lý CSS.
├── vite.config.js             # Cấu hình Vite cho React frontend (thiết lập base path cho Electron).
├── index.html                 # Điểm khởi đầu của ứng dụng React.
├── dist-package/              # Thư mục chứa file cài đặt .exe sau khi đóng gói.
└── src/
    ├── main/
    │   └── main.js            # Main Process của Electron (tải tệp, quản lý tiến trình, đọc file log, IPC handlers).
    ├── preload/
    │   └── preload.js         # Cầu nối an toàn contextBridge truyền tải API từ Main sang Renderer.
    └── renderer/
        ├── main.jsx           # Điểm khởi tạo React.
        ├── App.jsx            # Giao diện chính React (wizard cài đặt, radar phần cứng, dashboard quản trị).
        ├── index.css          # Định nghĩa Tailwind base, custom scrollbar và media query đồng bộ theme Windows.
        └── modelsData.js      # Danh mục dữ liệu mô hình AI Ollama và quy tắc tính tương thích phần cứng.
```

---

## 🚀 Hướng Dẫn Vận Hành & Đóng Gói (Development Guide)

### 1. Chuẩn bị môi trường phát triển
Đảm bảo bạn đã cài đặt Node.js trên máy tính phát triển. Tiến hành cài đặt dependencies:
```powershell
npm install
```

### 2. Khởi chạy môi trường phát triển (Dev Mode)
Khởi chạy đồng thời Vite dev server cho React frontend và ứng dụng Electron:
```powershell
npm run dev
```

### 3. Đóng gói ứng dụng thành file cài đặt `.exe`
Đóng gói toàn bộ ứng dụng thành một tệp cài đặt Windows Setup `.exe` duy nhất (sử dụng cấu hình NSIS, không tạo file zip) bằng lệnh:
```powershell
npm run dist
```
Sau khi hoàn thành, file cài đặt sẽ nằm tại thư mục `dist-package/EZ-OpenClaw Setup 1.0.0.exe`. Khi chạy file này, ứng dụng sẽ tự động cài đặt và tạo phím tắt trên Desktop/Start Menu.
